...
function install_php() {
    ...
    if [ ! -f "php-5.6.14.tar.gz" ];then 
        wget --no-check-certificate -O php-5.6.14.tar.gz http://cn2.php.net/get/php-5.6.14.tar.gz/from/this/mirror
    fi
    ...
    if [[ "$?" -ne 0 ]];then
        watchdog "Not found directory 'php-5.6.14'" ERROR
        exit 1
    fi
...
function init_php_ini() {
    local php_src_dir=${TAR_SRC_DIR}/php-5.6.14
