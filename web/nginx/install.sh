...
if [ ! -f "nginx-1.12.1.tar.gz" ];then 
    wget --no-check-certificate -O  nginx-1.12.1.tar.gz http://nginx.org/download/nginx-1.12.1.tar.gz 
fi

tar -xzvf nginx-1.12.1.tar.gz && cd nginx-1.12.1

if [[ "$?" -ne 0 ]];then
    echo "not found directory nginx-1.12.1"
    exit 1
fi
