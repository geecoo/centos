...
function install_apache() {
    ...
    if [ ! -f "httpd-2.4.17.tar.gz" ];then 
        wget --no-check-certificate -O  httpd-2.4.17.tar.gz http://mirrors.cnnic.cn/apache/httpd/httpd-2.4.17.tar.gz    
    fi
    ...
    if [[ "$?" -ne 0 ]];then
        echo "not found directory httpd-2.4.17  ...no"
        exit 1
    fi
